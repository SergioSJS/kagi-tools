...
class KagiSearch:
    ...
    def search(self, query: str, debug: bool = False) -> dict:
        ...
        try:
            ...
            if debug:
                ...
                print("📄 HTML salvo em debug_kagi.html")
    ...
    def _parse_html(self, html: str, debug: bool = False) -> list:
        ...
        result_divs = soup.find_all(
            "div", class_=lambda x: x and "search" in str(x).lower() and "result" in str(x).lower()
        )

        # Tentativa 2: Procurar por elementos com data-testid
        if not result_divs:
            result_divs = soup.find_all(
                ["div", "li"], attrs={"data-testid": lambda x: x and "result" in str(x).lower()}
            )
        ...
        if not result_divs:
            ...
            for div in all_divs:
                ...
                has_external_link = any(
                    ...
                    and "kagi.com" not in link.get("href", "")
                    for link in links
                )
        ...
        for div in result_divs:
            ...
            for link in div.find_all("a", href=True):
                ...
                if (
                    ...
                    and "kagi.com" not in href
                    and "wolframalpha.com" not in href
